  ...
  <div>
    ...
    <div style="border-bottom:1px solid #ddd;margin:20px 0">
      ...
      <div class="countproperty">
        ...
        <p>2.占消费总额的
          <InputNumber @on-change="Proportion_Terms" :value="countfilters.odlrankcalc" :min="0" :step="0.1"></InputNumber>%的消费占比</p>
        <p>3.会员日 &emsp;&emsp;调用全局会员日：
          ...
          <span v-if="countfilters.vip_days_input">
            会员日的消费占其总消费的<InputNumber  @on-change="vip_days_Terms" size="small" :value="countfilters.vip_days_num" :min="0" :step="0.1"></InputNumber>%
          </span>
        </p>
        <p>4.毛利额 &emsp;&emsp;添加毛利额：
          ...
          <span v-if="countfilters.profit.input">
            占整体会员毛利额的排名占比<InputNumber  @on-change="profit_Terms" size="small" :value="countfilters.profit.num" :min="0" :step="0.1"></InputNumber>%
          </span>
        </p>
      </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      countfilters: {
        ...
        vip_days_num:0,//会员日消费占比 
        vip_days_num2:0,//会员日消费占比 
        profit:{
          num:0,
          num2:0,
          input:false,
  ...
  methods: {
    ...
    Edit_router(item) {
      ...
      //2、差值条件
      if(item.multerms[0].reduceitem.minuend[0] && item.multerms[0].reduceitem.subtractor[0]){
    ...
    //是否添加毛利额占比
    profit_show(val) {
      this.countfilters.profit.num2 = 0
      this.countfilters.profit.num = 0
      if(val){
    ...
    //毛利额
    profit_Terms(val){
      this.countfilters.profit.num = val
      this.countfilters.profit.num2 = val / 100
    ...
    handleSubmit() {
      ...
      cafilters.forEach((value, index) => {
        let valueA = value
        let indexA = index
        this.allcount.forEach((value, index) => {
            if(this.allcount[index].field != 'vip_days'){
              valueA.field = this.allcount[index].field
              valueA.value = aa[indexA]
            }
        })
      })
     // console.log(cafilters)
      ...
      if(this.countfilters.profit.input ){
        if(this.countfilters.profit.num2 > 0){
          this.cityfilter.gross_profit_ranking = this.countfilters.profit.num2
        }
      }else{
          delete this.cityfilter.gross_profit_ranking
        }
      //console.log(JSON.stringify(this.cityfilter))
      ...
      if (!ff && filtersShow) {
        ...
        this.drug4ill != {} ||
        this.DateFilter[0] ||
        this.subtract.ifshow || profit.input || vip_days_input ||
        cafilters[0] ||
        cheap_drug
      ) {
        this.multerms.multerms.push(this.cityfilter)
    ...
    handleReset(val) {
      ...
      this.DateFilter = []
      this.$options.methods.handleAdd.bind(this)()
       this.$options.methods.Lately_Month_Terms.bind(this)(0)
       this.$options.methods.Proportion_Terms.bind(this)(0)
       this.$options.methods.Mvip_data_show.bind(this)('否')
       this.$options.methods.profit_show.bind(this)('否')
